...
def fit_predict_classification(X,y,Xtest,ytest):
    #Linear SVC with hinge loss
    metric="average_precision"
    param={'C':[100,10,1,0.1,0.01,0.001,0.0001]}
    model=GridSearchCV(svm.LinearSVC(loss='hinge',class_weight='balanced'),param,scoring=metric)
    model.fit(X,y)
    ...
    return {'ypred': ypred, 'ytest': ytest, 'auprc': metrics.average_precision_score(ytest,ypred)}
